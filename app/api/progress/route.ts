  ...
  try {
    ...
    if (supabase) {
      ...
      if (error) {
        ...
        completedTopics = Array.isArray(data.completed_topics) ? data.completed_topics : [];
        activeTopicId = data.active_topic_id || null;
        selectedRole = data.selected_role || null;
        
        let isDoneOnboarding = Boolean(data.has_completed_onboarding);
        if (
          !isDoneOnboarding &&
          ((data.selected_role && data.selected_role !== "all") ||
            (Array.isArray(data.completed_topics) && data.completed_topics.length > 0) ||
            (data.onboarding_data && Object.keys(data.onboarding_data).length > 0))
        ) {
          isDoneOnboarding = true;
        }

        hasCompletedOnboarding = isDoneOnboarding;
    ...
    if (!fetchedFromSupabase) {
      ...
      if (typeof meta?.hasCompletedOnboarding === "boolean") {
        ...
        (meta?.onboardingData && Object.keys(meta.onboardingData).length > 0)
      ) {
...
export async function POST(req: Request) {
  try {
    ...
    // Fetch user details from Clerk to store username & email in database
    ...
    try {
      ...
      userEmail = clerkUser.primaryEmailAddress?.emailAddress || clerkUser.emailAddresses?.[0]?.emailAddress || null;
    } catch (err) {
      console.warn("Could not fetch Clerk user details for progress sync:", err);
    }

    let hasCompletedOnboardingVal = Boolean(hasCompletedOnboarding);

    // 1. Try Upserting into Supabase
    let supabaseSuccess = false;
    const supabase = getSupabaseAdmin();
    if (supabase) {
      if (!hasCompletedOnboardingVal) {
        const { data: existing } = await supabase
          .from("user_progress")
          .select("has_completed_onboarding, selected_role, completed_topics, onboarding_data")
          .eq("user_id", userId)
          .maybeSingle();

        if (existing?.has_completed_onboarding) {
          hasCompletedOnboardingVal = true;
        } else if (
          (selectedRole && selectedRole !== "all") ||
          completedTopics.length > 0 ||
      ...
      const { error } = await supabase.from("user_progress").upsert(
        {
          ...
          has_completed_onboarding: hasCompletedOnboardingVal,
    ...
    // 2. Also sync to Clerk metadata as backup
    try {
      const clerk = createClerkClient({ secretKey: process.env.CLERK_SECRET_KEY });
      ...
      // Ignored if secret key not present
    }
